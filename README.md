...
如果真实章节未知，skill 会要求 agent 询问用户或查资料，而不是凭空编造。页码、讲次、题号未知时，默认仍可按真实章节安排，不强行补细节。
...
如果资料没有真实章节目录，agent 必须先查权威来源，或要求用户提供目录、PDF、截图、OCR 文本、课程列表。不能继续凭空拆章节。章节目录可用后，就可以按章节规划；题号和讲次不作为默认硬门槛。
...
`--bind-tasks` 会把当前档案中已知的王道 408 任务绑定到对应章节。这个导入只相信 Obsidian 里已有的章节链接；如果笔记没有页码、讲次、题号，系统会保留为空，不会自动编造，也不会阻止章节级计划。
...
- 阻塞诊断：用于有任务依赖的计划，例如“先完成 A 才能做 B”。没有依赖链时可以忽略。
- 408 专项模型：只用于计算机考研 408。非 408 考试不会依赖它。
